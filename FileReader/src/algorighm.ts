...
function findSimilarPoints(routeA: Route, routeB: Route) {
    ...
    console.log("Distance:", distanceSum * 2);

    let overlapLength = 0;
    for (let i = firstOverlapIndex; i < edgesB.length; i++) {
        overlapLength += distance(edgesB[i]);
    }
    console.log("Overlap length:", overlapLength);


    return { extraDistance: distanceSum * 2, length: overlapLength };
}
